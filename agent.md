...
- **데이터 소스 (로컬 업로드)**
  - 입력: 사용자 PC에서 PDF 업로드만 허용 → 세션별 임시 워크스페이스 생성·정리.
  ...
  - 페이지 범위: 대용량 문서는 필요 페이지 범위(예: 30~80p)만 파싱/임베딩 옵션 제공.
...
- **PySide6 GUI (선택 확정)**
  - 레이아웃: 상단 탭 3개(업로드/질의/설정) + 하단 PDF 뷰어(페이지 이동·bbox 하이라이트), 별도 로그 패널.
  - 업로드 탭: 세션 선택/생성, PDF 선택, 페이지 범위 필수 입력, `replace` 체크박스, 메타(product_name/model_id/version/region) 입력, 진행률/로그 표시.
  - 질의 탭: 질문 입력, 비교 모드(멀티셀렉트 콤보로 제품/모델 선택), 고급 설정 슬라이더(`top_k`, `rerank_top_n`), 리랭커 토글(기본 On), 결과(불릿/표, Notes 열 포함), 출처 태그 클릭 시 새 탭으로 PDF 하이라이트.
  - 설정 탭: `.env` 경로 지정/재로딩(기본 실행 디렉터리), Qdrant 상태 표시(HTTP 우선, 실패 시 임베디드 전환 뱃지).
  - 로그: 파싱/임베딩/검색/에러 로그를 별도 패널에 표시.
